...
def fetch_and_store_economic_data():
    ...
    query = "INSERT INTO economyhistory (date, inflation, ECBInterest, NL_Unemployment, NL_ConsumerConfidence, EUR_USD_ExchangeRate, NL_GDPGrowth, NL_RetailSales, NL_IndustrialProduction, NL_Bankrupcies) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    inputs = [timestamp, inflation, ECBIR, unemployment, consumerConfidence, EUR_USD_ExchangeRate, GDPGrowth, NL_RetailSales, NL_IndustrialProduction, NL_Bankrupcies]
    securityFilters = [r'^\d{4}-\d{2}-\d{2}$|^$|^None$', r'^-?\d+(\.\d+)?$|^$|^None$', r'^-?\d+(\.\d+)?$|^$|^None$', r'^-?\d+(\.\d+)?$|^$|^None$', r'^-?\d+(\.\d+)?$|^$|^None$', r'^-?\d+(\.\d+)?$|^$|^None$', r'^-?\d+(\.\d+)?$|^$|^None$', r'^-?\d+(\.\d+)?$|^$|^None$', r'^-?\d+(\.\d+)?$|^$|^None$', r'^-?\d+(\.\d+)?$|^$|^None$']
...
def computeExpectedDividends(ticker):
    ...
    if not stock_id_result:
        ...
        if allreadyExist == None:
            ...
            end_date = datetime(today.year, 1, 1) - timedelta(days=1)  # End is Dec 31 of last year
            ...
            # Yahoo finance uses the announcement date, while we want the payment date, which usually is a month later
            incremented_months = [(month % 12) + 1 for month in dividend_months]  
            incremented_month_names = [datetime(today.year, month, 1).strftime('%B') for month in incremented_months]

            for month in incremented_month_names:
                    query = "INSERT INTO dividendExpectations (stockID, expectedMonth, expectedDividendPerShare) VALUES (?, ?, ?)"
                    inputs = [int(stock_id_result[0]), month, float(average_dividend)] 
                    securityFilters = [r'^\d+$', r'^[A-Za-z]{1,10}$', r'^\d+\.\d+$'] # Patterns for integer, month name, and float
                    insert_data(query,inputs,securityFilters)
    ...
    for _, row in filtered_df.iterrows():
        if row['ticker'] not in tickers:
            if is_ticker_available(row['ticker']):
                ...
                query = "INSERT INTO Stocks (ticker, stockname, Exchange, DateAdded, Status, assetType)VALUES (?, ?, ?, ?, ?, ?)"
                inputs = [row['ticker'], row['stockname'], row['Exchange'], row['DateAdded'], row['Status']]
                securityFilters = [r'^[A-Za-z0-9]{1,20}$', r'^[A-Za-z0-9\s\.,;\$\%\+\-\=!&/\\]*$|^None$', r'^[A-Za-z0-9\s\.,;\$\%\+\-\=!&/\\]*$|^None$', r'^\d{4}-\d{2}-\d{2}$|^$|^None$', r'^[a-z]*$|^None$',r'^[A-Za-z]+$']
                insert_data(query, inputs, securityFilters)
            else:

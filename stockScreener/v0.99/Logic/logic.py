...
def get_dividend_expectations():
    query = "SELECT Stocks.ticker, dividendExpectations.stockID, dividendExpectations.expectedMonth, dividendExpectations.expectedDividendPerShare, dividendExpectations.id FROM dividendExpectations JOIN Stocks ON dividendExpectations.stockID = Stocks.id"
...
def insert_dividend_expectation(myRequest):
    ...
    inputs = [int(myRequest.form['stock_id']), myRequest.form['month'], float(myRequest.form['expected_dividend'])] 
...
def insert_transaction(myRequest):
    ...
    if not stock_id_result:
        ...
        inputs = [int(stock_id_result[0]), myRequest.form['date'], int(myRequest.form['quantity']), myRequest.form['currency'], float(myRequest.form['stock_price']), myRequest.form['type'], float(myRequest.form['transaction_fee'])]
        securityFilters = [r'^\d+$', r'^\d{4}-\d{2}-\d{2}$', r'^\d+$', r'^(EUR|USD)$', r'^\d+\.\d{1,2}$', r'^(Buy|Sell)$',r'^\d+\.\d{1,2}$']
        insert_data(query,inputs,securityFilters)
...
def updateExpectedDividend(divID, expectedMonth, divPerShare):
    query="UPDATE dividendExpectations SET expectedMonth=?, expectedDividendPerShare=? where id = ?"
    inputs= [expectedMonth, divPerShare, divID]
    securityFilters = [r'^[a-zA-Z]{3,12}$', r'^\d+(\.\d+)?$', r'^\d+$']
    update_data(query,inputs,securityFilters)
